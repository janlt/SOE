    ...
    uint32_t cnt)
{
    ...
    for ( uint32_t i = 0 ; i < cnt ; i++ ) {
        sprintf(key_buf, "%u", num + i);
        args.key = key_buf;
        args.key_len = strlen(key_buf);
        sprintf(data_ptr, "%u%u%u%u", num, 2*num, 3*num, 4*num);
...
static void WriteThreadAsyncStoreN(LocalArgsDescriptor *argsptr)
{
    ...
    } catch (const KvsFacade::Exception &e) {
        std::cout << "KvsFacade::Exception: " << e.what() << std::endl << std::flush;
...
{
    std::cout << prog << " cluster space store store_cnt -{c|w|d|n|t|s|e|f|k|l|z|u|x|g} {key|overwr_dup} {data|end_key} cluster_id space_id store_id {read_cnt|write_cnt} data_size" << std::endl << std::flush;
