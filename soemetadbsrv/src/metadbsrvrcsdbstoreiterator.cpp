...
StoreIterator::StoreIterator(StoreIterator::eType _type)
    :rcsdb_iter(0),
     ...
     end_key_buf(0),
     comb_end_key_buf(0),
     start_key_buf_size(0),
     end_key_buf_size(0),
     comb_end_key_buf_size(0),
     key_pref_buf_size(0),
     dir_forward(true),
     ...
     type(_type)
{
    start_key_buf_size = 64;
    start_key_buf = new char[start_key_buf_size];
    end_key_buf_size = 64;
    end_key_buf = new char[end_key_buf_size];

    if ( type == StoreIterator::eType::eStoreIteratorTypeSubset ) {
        comb_end_key_buf_size = 4000;
        comb_end_key_buf = new char[comb_end_key_buf_size];
    }

    key_pref_buf_size = 64;
    key_pref_buf = new char[key_pref_buf_size];

    ::memset(key_pref_buf, '\0', key_pref_buf_size);
}
...
StoreIterator::StoreIterator(Store &_store,
    ...
    uint32_t _i)
        :rcsdb_iter(0),
         ...
         type(_type)
{
    start_key_buf_size = _start_key.size() + 64;
    start_key_buf = new char[start_key_buf_size];
    end_key_buf_size = _end_key.size() + 64;
    end_key_buf = new char[end_key_buf_size];

    if ( type == StoreIterator::eType::eStoreIteratorTypeSubset ) {
        comb_end_key_buf_size = _end_key.size() + 40000;
        comb_end_key_buf = new char[comb_end_key_buf_size];
    }

    key_pref_buf_size = _key_pref.size() + 64;
    key_pref_buf = new char[key_pref_buf_size];
    ...
    if ( type == StoreIterator::eType::eStoreIteratorTypeSubset ) {
        ...
        ::memcpy(comb_end_key_buf, end_key.data(), end_key.size());
        ::memset(comb_end_key_buf + end_key.size(), 0xff, comb_end_key_buf_size - end_key.size());
        comb_end_key = Slice(comb_end_key_buf, comb_end_key_buf_size);
    }
}
...
StoreIterator::~StoreIterator()
{
    ...
    delete [] start_key_buf;
    delete [] end_key_buf;

    if ( type == StoreIterator::eType::eStoreIteratorTypeSubset ) {
        delete [] comb_end_key_buf;
    }
...
{
    if ( type == StoreIterator::eType::eStoreIteratorTypeSubset ) {
        rcsdb_iter->SeekForPrev(comb_end_key);
    } else {
