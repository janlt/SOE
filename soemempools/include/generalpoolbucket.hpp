/*
 * generalpoolbucket.hpp   (C) Copyright Jan Lisowiec, 2003-2017
 *
 */

/*
 * generalpoolbucket.hpp
 *
 *  Created on: Dec 21, 2016
 *      Author: Jan Lisowiec
 */

#ifndef SOE_SOE_SOE_MEMPOOLS_INCLUDE_GENERALPOOLBUCKET_HPP_
#define SOE_SOE_SOE_MEMPOOLS_INCLUDE_GENERALPOOLBUCKET_HPP_


#include "allocstrategy.hpp"
#include "generalpoolbuffer.hpp"
...
private:
    ...
    GeneralPoolBucket<T, Alloc> & operator=(
            const GeneralPoolBucket<T, Alloc> &right);
...
inline GeneralPoolBucket<T, Alloc>::GeneralPoolBucket() :
        ...
        numBuffers(0),
        stamp(0),
        totalMax(MAX_BUCKET_BUFFERS * MAX_BUFFER_SLOTS),
        currentMax(0),
        objSize(Alloc::objectSize()),
        padding(2 * OBJ_PADDING),
        stackTraceFile(0)
#ifdef _MEM_STATS_
        ,getRequests(0UL),
        putRequests(0UL),
        memAllocs(0UL),
        memDeallocs(0UL),
        stackTraceThreshold(1000UL),
        stackTrace(false),
        tracePoolNameFlag(false)
#endif
{
#ifdef _MM_DEBUG_
#ifndef _MM_GMP_ON_
...
template<class T, class Alloc>
inline T * GeneralPoolBucket<T, Alloc>::get()
{
    // set the lock on
...
inline T * GeneralPoolBucket<T, Alloc>::getNoLock()
{
    ...
    if ( stackTrace == true ) {
        if ( memAllocs > stackTraceThreshold ) {
            if ( tracePoolNameFlag == false || !strncmp(rttiName, tracePoolName, sizeof(tracePoolName)) )
            traceStack();
        }
    }
#endif
    ...
    if (!tmp_obj) {
        // error - abort
#ifndef _MM_GMP_ON_
        cerr << "Null returned from slot, b_idx: " << buf_idx << " s_idx: "
                << slot_idx << endl << flush;
        cerr << "-> Bucket ";
#else
    ...
    // advance the tail, there are objects there because of they never drop under 10
    tail++;
    tail %= currentMax;
...
inline void GeneralPoolBucket<T, Alloc>::putNoLock(T *obj_)
{
    ...
    if ( empty != true ) {
        ...
        if ( tmp_obj == obj_ ) {
#ifndef _MM_GMP_ON_
            cerr << "Object already returned, o_b_idx: " << o_buf_idx << " o_s_idx: " << o_slot_idx << endl << flush;
            cerr << "-> Bucket ";
#else
            fprintf(stderr, "Object already returned, o_b_idx: %d o_s_idx: %d\n", o_buf_idx, o_slot_idx);
            fflush(stderr);
            fprintf(stderr, "-> Bucket");
#endif
            dumpNoLock();
...
#ifndef _MM_GMP_ON_
        cerr << "Slot should be NULL, b_idx: " << buf_idx << " s_idx: "
                << slot_idx << endl << flush;
        cerr << "-> Bucket ";
#else
...
inline void GeneralPoolBucket<T, Alloc>::make(unsigned int count_)
{
    for (unsigned int cnt = 0; cnt < count_; cnt++) {
        ...
        *((unsigned int *)((char *)tmp_obj - OBJ_PADDING)) = stamp;
        ...
        << endl << flush;
#else
        fprintf(stderr, "GeneralPoolBucket<T,Alloc>::make() tmp_obj: %x tail: %d head: %d currentMax: %d numBuffers: %d stamp: %d os: %d full: %d empty: %d\n",
                tmp_obj, tail, head, currentMax, numBuffers, stamp, o_stamp, full, empty);
        fflush(stderr);
#endif
#endif
...
inline void GeneralPoolBucket<T, Alloc>::dumpNoLock()
{
    ...
    cerr << " , obj size: " << objSize << " num of buffers: " << numBuffers
            ...
            << endl << flush;
#else
    fprintf(stderr, " , obj size: %d num of buffers: %d objs in: %d tail: %d head: %d\n",
    ...
    for (unsigned int cnt = tail; cnt != head;) {
        ...
        if (tmp_obj) {
#ifndef _MM_GMP_ON_
            unsigned int o_stamp = *((unsigned int *) ((char *) tmp_obj
                    + objSize));
#else
            unsigned int o_stamp = *((unsigned int *)((char *)tmp_obj - OBJ_PADDING));
#endif
            ...
            unsigned int o_bucket_size = o_stamp & BUCKET_SIZE_MASK;
            unsigned int o_bucket_number = (o_stamp >> BUCKET_NUMBER_SHIFT)
                    & BUCKET_NUMBER_MASK;

#ifndef _MM_GMP_ON_
            cout << " -> Obj, cnt: " << cnt << " o_stamp: " << o_stamp
                    << " o_bs: " << o_bucket_size << " o_bn: "
                    << o_bucket_number << endl << flush;
#else
...
template<class T, class Alloc>
inline void GeneralPoolBucket<T, Alloc>::setStrategy(
        const AllocStrategy &strat_)
{
...
inline void GeneralPoolBucket<T, Alloc>::makeBuffer()
{
...
#ifndef _MM_GMP_ON_
        cerr << "No more buffers can be allocated " << " See Jan Lisowiec"
                << endl << flush;
#else
...
#ifndef _MM_GMP_ON_
            cerr << " ===> Wrong entry in bucket: " << bucketNum << " o_bs: " << o_bucket_size
            << " o_bn: " << o_bucket_number << " entry: " << cnt << endl << flush;
#else
...
#ifndef _MM_GMP_ON_
            cout << " ===> Obj NULL entry: " << cnt << " tail: " << tail << " head: " << head << endl << flush;
#else
    ...
    " head: " << head << " numBuffers: " << numBuffers << " bucketNum: " <<
    bucketNum << " full: " << full << " empty: " << empty << endl << flush;
#endif
#endif
...
inline void GeneralPoolBucket<T, Alloc>::stats(FILE &file_) {
    ...
    fprintf(&file_, " -> Bucket num: %d getCnt: %ld putCnt: %ld memAllocs: %ld memDeallocs: %ld size: %ld",
...
#endif /* SOE_SOE_SOE_MEMPOOLS_INCLUDE_GENERALPOOLBUCKET_HPP_ */
