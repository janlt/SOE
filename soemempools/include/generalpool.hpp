/*
 * generalpool.hpp   (C) Copyright Jan Lisowiec, 2003-2017
 *
 */
...
#ifndef SOE_SOE_SOE_MEMPOOLS_INCLUDE_GENERALPOOL_HPP_
#define SOE_SOE_SOE_MEMPOOLS_INCLUDE_GENERALPOOL_HPP_


#include "classmemoryalloc.hpp"
#include "classalloc.hpp"
#include "generalpoolbase.hpp"
#include "generalpoolbucket.hpp"
#include "defaultallocstrategy.hpp"
#include "globalmemoryalloc.hpp"
...
public:
    GeneralPool (unsigned short id_, const char *name_ = 0, const AllocStrategy &strategy_ = DefaultAllocStrategy());
    GeneralPool (const char *name_ = 0, const AllocStrategy &strategy_ = DefaultAllocStrategy());
    virtual ~GeneralPool();
    virtual void manage ();
    T * get ();
...
template <class T, class Alloc>
inline GeneralPool<T,Alloc>::GeneralPool (unsigned short id_, const char *name_, const AllocStrategy &strategy_)
     :GeneralPoolBase(id_, name_),
...
#ifdef _MM_DEBUG_
#ifndef _MM_GMP_ON_
    cout < "NAME: " << ti.name() << " comb name: " << comb_name << endl << flush;
#else
...
// set the rttiName in the base class
    ...
    << endl << flush;
#else
    fprintf(stderr, "GeneralPool<T,Alloc>::GeneralPool name: %s id: %d objSize: %d\n",
...
template <class T, class Alloc>
inline GeneralPool<T,Alloc>::GeneralPool (const char *name_, const AllocStrategy &strategy_)
     :GeneralPoolBase(0, name_),
     strategy(strategy_.copy()),
     counter(0),
     objSize(Alloc::objectSize())
{
...
template <class T, class Alloc>
inline GeneralPool<T,Alloc>::~GeneralPool()
{
    delete strategy;
...
template <class T, class Alloc>
inline void GeneralPool<T,Alloc>::manage ()
{
#ifdef _MM_DEBUG_
#ifndef _MM_GMP_ON_
...
template <class T, class Alloc>
inline T * GeneralPool<T,Alloc>::get ()
{
    ...
    << endl << flush;
#else
    fprintf(stderr, "GeneralPool<T,Alloc>::get, stamp: %d bs: %d bn: %d objSize: %d name: %s\n",
...
inline void GeneralPool<T,Alloc>::put (T *obj_)
{
    ...
    unsigned int bucket_number = (stamp >> BUCKET_NUMBER_SHIFT) & BUCKET_NUMBER_MASK;
    ...
    << endl << flush;
#else
    fprintf(stderr, "GeneralPool<T,Alloc>::put, stamp: %d bs: %d bn: %d objSize: %d name: %s\n",
...
#ifndef _MM_GMP_ON_
        cerr << "Object returned to the wrong pool, obj size: " << bucket_size << " bucket size: " <<
        objSize << " name: " << getName() << " id: " << getId() << " obj_: " << obj_ << endl << flush;
#else
        fprintf(stderr, "Object returned to the wrong pool, obj size: %d bucket size: %d name: %s id: %d obj_: %x\n",
...
#ifndef _MM_GMP_ON_
        cerr << "Bucket number out of range, obj size " << bucket_size << " bucket size: " <<
        objSize << " name: " << getName() << " id: " << getId() << " obj_: " << obj_ << endl << flush;
#else
        fprintf(stderr, "Bucket number out of range, obj size: %d bucket size: %d name: %s id: %d obj_: %x\n",
        bucket_size, objSize, getName(), getId(), obj_);
        fflush(stderr);
#endif
...
template <class T, class Alloc>
inline void GeneralPool<T,Alloc>::dump ()
{
#ifndef _MM_GMP_ON_
    cerr << "Dumping name: " << getName() << " id: " << getId() << " bucketSize: " << objSize << endl << flush;
#else
...
inline void GeneralPool<T,Alloc>::stats (FILE &file_)
{
    ...
    getName(), getRttiName(), getId(), objSize);
...
#endif /* SOE_SOE_SOE_MEMPOOLS_INCLUDE_GENERALPOOL_HPP_ */
